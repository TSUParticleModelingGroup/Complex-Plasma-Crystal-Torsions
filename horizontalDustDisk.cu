...
double 	Dt;
int 	DrawRate;
float	LineWidthDustPointCharge;
float	LineWidthParentChild;
int	ArrowheadShape;
...
int FrustrumOrthoganal; // Frustrum = 1, Orthoganal = 0.
...
void readSimulationParameters()
{
	...
	if(data.is_open() == 1)
	{
		...
		getline(data,name,'=');
		data >> LineWidthDustPointCharge;
		
		getline(data,name,'=');
		data >> LineWidthParentChild;
	...
	printf("\n LineWidthDustPointCharge....................... %f width of the line between a dust grain and its point charge", LineWidthDustPointCharge);
	printf("\n LineWidthParentChild........................... %f width of the line between a parent and its child", LineWidthParentChild);
	printf("\n ArrowheadShape................................. %d shape of the arrowhead from parent to its child", ArrowheadShape);
...
void PutConstantsIntoOurUnits()
{
	...
	{
		printf("\n Bad dust distribution type. It should be 0, or 1.");
		printf("\n Check your setup file. Good Bye.");
		exit(0);
	}
	...
	printf("\n Dt................................... %e", Dt);
	printf("\n DrawRate............................. %d", DrawRate);
	printf("\n LineWidthDustPointCharge............. %f", LineWidthDustPointCharge);
	printf("\n LineWidthParentChild................. %f", LineWidthParentChild);
	if (ArrowheadShape == 0)
	{ 
		printf("\n Using no arrowheads.");
	}
	else if (ArrowheadShape == 1)
	{
		printf("\n Using razor shaped arrowheads.");
	}
	else if (ArrowheadShape == 2)
	{
		printf("\n Using conical shaped arrowheads.");
	}
	else
	{
		printf("\n Bad arrowhead type. It should be 0, 1, or 2.");
		printf("\n Check your setup file. Good Bye.");
		exit(0);
	}
...
void drawPicture()
{
	...
	for(int i = 0; i < NumberOfDustParticles; i++)
	{
		...
		if(DustViewingAids)
		{
			// Line between Dust and its point charge
			glLineWidth(LineWidthDustPointCharge);
			...
			glLineWidth(LineWidthParentChild);
			for (int j = 0; j < MAX_NUMBER_CHILDREN; j++)
			{
				if(DustParentChildCPU[i].childId[j] != -1) 
				{
					...
					if(ArrowheadShape == 1)
					{
						...
						float x1 = DustPositionCPU[i].x;
						float y1 = DustPositionCPU[i].y;
						float z1 = DustPositionCPU[i].z;
						float x2 = DustPositionCPU[compId].x;
						float y2 = DustPositionCPU[compId].y;
						float z2 = DustPositionCPU[compId].z;
						...
						float arrowLength = 0.1f;  // Length of the arrowhead
						float arrowWidth = 0.03f;  // Width of the arrowhead

						// Create two perpendicular vectors to (vx, vy, vz) for the arrowhead
						float ux = -vy;
						float uy = vx;
						float uz = 0;
						...
						float x3 = x2 - arrowLength * vx + arrowWidth * ux;
						float y3 = y2 - arrowLength * vy + arrowWidth * uy;
						float z3 = z2 - arrowLength * vz + arrowWidth * uz;

						float x4 = x2 - arrowLength * vx - arrowWidth * ux;
						float y4 = y2 - arrowLength * vy - arrowWidth * uy;
						float z4 = z2 - arrowLength * vz - arrowWidth * uz;

						glBegin(GL_TRIANGLES);
						    // First triangle (one side of the arrowhead)
						    glVertex3f(x2, y2, z2); // Tip of the arrowhead
						    glVertex3f(x3, y3, z3); // One base corner of the arrowhead
						    glVertex3f(x4, y4, z4); // Other base corner of the arrowhead
						glEnd();
					}
					else if(ArrowheadShape == 2)	
					{					
						...
						// Define the size of the arrowhead (cone)
						float coneHeight = 0.1f;  // Height of the cone
						float coneRadius = 0.03f;  // Radius of the cone's base
						int numSegments = 20;  // Number of segments to approximate the cone's base

						// Calculate the base center of the cone
						float cx = x2 - coneHeight * vx;
						float cy = y2 - coneHeight * vy;
						float cz = z2 - coneHeight * vz;

						// Generate a perpendicular vector for the cone's base
						float ux, uy, uz;
						if (fabs(vx) > fabs(vy)) {
						    ux = -vz;
						    uy = 0;
						...
						// Cross product to get another perpendicular vector
						float wx = vy * uz - vz * uy;
						float wy = vz * ux - vx * uz;
						float wz = vx * uy - vy * ux;
						...
						glBegin(GL_TRIANGLE_FAN);
							...
							float px = cx + coneRadius * (cos(angle) * ux + sin(angle) * wx);
							float py = cy + coneRadius * (cos(angle) * uy + sin(angle) * wy);
							float pz = cz + coneRadius * (cos(angle) * uz + sin(angle) * wz);
							glVertex3f(px, py, pz);
						    }
						glEnd();
...
__global__ void groupDust(float4 *dustPos, float cutOff, float4 *ionWake, dustParentChildStructure *dustParentChildGPU, int numberOfParticles, float baseIonWakeLength, float baseIonWakeFractionalCharge)
{
	...
	for (int k = 0; k < MAX_NUMBER_CHILDREN; k++)
	{
		...
		{
			break; // if there are no children w/in cutoff here, there won't be in the next.
		}
	}
								...
								float chargeExchangeFraction, int numberOfParticles)
{
	...
	float initialLength = -ionWake[parentId].y;
	...
	for (int k = 0; k < MAX_NUMBER_CHILDREN; k++)
	{
		...
		ionWakeLength -= initialLength * (1.0f - linearReduction) * angularAdjustment;
		...
		temp = initialCharge * (1.0f - quadraticReduction) * angularAdjustment;
		ionWakeCharge -= temp;
	...
	if(0.0f < ionWakeCharge)
	{
		ionWake[parentId].w = ionWakeCharge;
	}
	else
	{
		ionWake[parentId].w = 0.0;
	}
	if(0.0f < ionWakeLength)
	{
		ionWake[parentId].y = -ionWakeLength; // Minus sign because it is below the dust.
	}
	else
	{
		// Note: If you turn on the useSelfPointCharge flag in the getForce function 
		// your program will have a divide by zero and it will crash. As is it kind of 
		// reduces the negative charge on the dust which is physically correct.
		ionWake[parentId].y = 0.0; 
	}
	
	// Setting the children dust values.
			  ...
			  float sheathHeight, float bottomPlateForcingParameter, float gravity, int numberOfParticles)
{
	...
	if(myId < numberOfParticles) // Making sure we don't work past the number of particles.
	{
		...
		// We just left it in, incase someone wanted to play around with it they could 
		// with very little code changes.
		int useSelfPointCharge = 0; // 1 in on, 0 is off
